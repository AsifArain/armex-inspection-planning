function [ wV,...
           ...
           tPreprocess ] = fVDReEstimatedHc( map_env,...
                                             FoV,...
                                             hc_current,...
                                             confs_executed_hot,...
                                             OBSenv,...
...
hc_current = round(hc_current);

%////////////////////////////////////////////////////////
% if hotspot is on the obstacle
%////////////////////////////////////////////////////////

if map_env(hc_current(1),hc_current(2)) ==0
    
    coverage_ind = find(map_env(:)==1);
    [cov_r,cov_c] = ind2sub(size(map_env),coverage_ind);
    
    dist = pdist2(hc_current,[cov_r,cov_c],'euclidean');
    [~,ind] = min(dist);
    
    shifted_ind = coverage_ind(ind);
    [shifted_r,shifted_c] = ind2sub(size(map_env),shifted_ind);
    hc_current = [shifted_r,shifted_c];
    
end
    


hotspots = sub2ind(size(map_env),hc_current(1),hc_current(2));

% [hotspot_r,hotspot_c] = ind2sub(size(map_env),hotspot);
hotspots_r = hc_current(1);
hotspots_c = hc_current(2);
...
executedConfsHot_ind = sub2ind(size(map_env),...
                               round(confs_executed_hot(:,1)-0.5),...
                               round(confs_executed_hot(:,2)-0.5));
executedConfsHot_orn = confs_executed_hot(:,3);
...
% profile off
% profile_name = 'visibility_profile_test';
% mkdir(profile_name)
...
if numel(executedConfsHot_ind) ~= numel(executedConfsHot_num)
    warning(['All the executed configurations used to estimated the hc '...
             'are not considered as already executed configurations for '...
             'the reconstruction of this hotspot'])
    warning(['Confs inds for the estimation of hc:    ',...
              num2str(executedConfsHot_ind')])
    warning(['Confs nums fixed in the reconstruction: ',...
              num2str(executedConfsHot_num)])
end

% 
% hold on;
% confCELL = confKept(4)
% [conf_r,conf_c] = ind2sub(size(map_env),confCELL);
% plot(conf_r,conf_c,'xb');
% 
% confCELL = confKept(88)
% [conf_r,conf_c] = ind2sub(size(map_env),confCELL);
% plot(conf_r,conf_c,'xb');
% 
% pause
...
% -- plot
if visualize == 1
figure('name','cand conf - max limit');
imshow(map_env','InitialMagnification',800); hold on;
set(gca,'YDir','normal'); colormap('gray'); caxis([-2 2])
map_candConf_ind = find(map_candConf(:)==1);
[map_candConf_r,map_candConf_c] = ind2sub(size(map_env),map_candConf_ind);
...
% % ----- cross angles
% disp('----- cross angles')
% conf_crossAngles = zeros(numel(Hot2Conf_Angles)); % matrix of cross angles between candidate conf.
% 
% % plot for debugging
...
%----------------------------------------------------
% coverage_ind = find(map_coverage); % --- original ---
coverage_ind = find(map_env);
...
%////////////////////////////////////////////////
%
% CANDIDATE CONFIGUARATIONS TO KEEP
%
%////////////////////////////////////////////////
...
%--- DEBUG
% [confKept_r,confKept_c] = ind2sub(size(map_env),confKept);
% h1 = plot(confKept_r,confKept_c,'xb','MarkerFaceColor','b');
% h2 = plot(hotspots_r,hotspots_c,'sr');
% fixedConfs_ind = executedConfsHot_ind;
% [fixedConfs_r,fixedConfs_c] = ind2sub(size(map_env),fixedConfs_ind);
% h3 = plot(fixedConfs_r,fixedConfs_c,'or');
% pause
% delete(h1)
% delete(h2)
% delete(h3)
...
% [hotspots_r,hotspots_c] = ind2sub(size(map_env),hotspots);
[conf_r,conf_c] = ind2sub(size(map_candConf),confKept);
desiredOrns = round(rad2deg(angle2Points([conf_r',conf_c'],...
                                         [hotspots_r(1),hotspots_c(1)])));
desiredOrns = desiredOrns + (desiredOrns==0)*360;
