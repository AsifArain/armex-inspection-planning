function [ C,...
           ...
           tSPP_rwl1,...
           tSPP_Comb,...
           tSPP ] = fPlan_GD_FullCoverage_RWL1( V,...
                                                FoVfaces,...
                                                map,...
                                                paramSPP )
%fSPP_rwl1_R5 is Sensor Placement Problem solution based on Reweighted l1 minimization.
% conv-SPP
...
% logs_rwl1  : [k,5][] Logs for each (k) iterations of re-weightes l1 minimization. Loged parameters are: iterat := iteration
%                     number, epsilon := epsilon to find next weights, improvCu := difference of uncertain conf from the last
%                     iterations, numCu := number of uncertain conf, and tItr := computation time for the iteration.
...
% paramSPP  : [][] .numHistIR := number of last iterations to keep in history table of improvement rate for uncertain conf.
%                  .maxIterat := maximum number of iterations for rwl1.
%                  .numCu     := maximum number of uncertain conf to terminate rwl1 search.
%                  .Prog      := program script to be used. 'CVX' for CVX, 'GRB' for gurobi, and 'MOT' for matlab
...
HistIR       = inf(paramSPP.numHistIR,1);    % history array of uncertain conf improvement rate for n iterations.
logs_rwl1    = zeros(paramSPP.maxIterat,5);  % data logging for each iteration of important parameters.
...
while ~cond
    ...
    cond       = (iterat>=paramSPP.maxIterat|numCu<=paramSPP.numCu|~any(HistIR));  % condition for the next iteration.
    % -----------------------------------------------------------------------------------------------------------------------
    disp(['Iteration: ',num2str(iterat),', Num of uncertain conf.: ',num2str(numCu),', Improvement rate: ',num2str(improvCu),...
        ', Time: ',num2str(tItr)]);    
