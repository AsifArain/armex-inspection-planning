...
switch para_.SensingSystem
    ...
    case 'simulation'
        ...
        for num = 1:conf_sequence_num
            meas_file = sprintf('measurement_conf%02d.mat',num);
            M_current = load([dir_.MeasurementLogs,meas_file]);
...
% %--- save mean map
% recon_file = sprintf('reconstruction_conf%02d.dat',conf_sequence_num);
...
thisfile = sprintf('reconstructionColorR_conf%02d.dat',conf_sequence_num);
fileID = fopen([dir_.Recon,thisfile],'wt'); fclose(fileID);
dlmwrite([dir_.Recon,thisfile],colR,'-append','delimiter',' ');
fileID = fopen([dir_.Recon,thisfile],'a'); fclose(fileID);
...
thisfile = sprintf('reconstructionColorG_conf%02d.dat',conf_sequence_num);
fileID = fopen([dir_.Recon,thisfile],'wt'); fclose(fileID);
dlmwrite([dir_.Recon,thisfile],colG,'-append','delimiter',' ');
fileID = fopen([dir_.Recon,thisfile],'a'); fclose(fileID);
...
thisfile = sprintf('reconstructionColorB_conf%02d.dat',conf_sequence_num);
fileID = fopen([dir_.Recon,thisfile],'wt'); fclose(fileID);
dlmwrite([dir_.Recon,thisfile],colB,'-append','delimiter',' ');
fileID = fopen([dir_.Recon,thisfile],'a'); fclose(fileID);
