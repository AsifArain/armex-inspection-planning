...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%------------------------------------------
% reconstruction map as received
%------------------------------------------
...
%*********************************************************************
[ estimated_sources ] =...
    fEstimatedGasSources( map_recon2,...
                          map_env,...
                          cellsize_env,...
                          OBSenv,...
                          para_ )
...
[ true_positives,...
  false_positives,...
  num_of_true_positives,...
  num_of_false_positives] = fTrueFalsePositives( true_sources,...
                                                 estimated_sources,...
                                                 map_env,...
                                                 para_ )
...
num_of_true_sources = size(true_sources,1);
...
positives_association_ind = dsearchn(true_sources,estimated_sources);
...
for i = 1:numel(tf_positives_association_ind)
    ...
    pt_from = true_sources(tf_positives_association_ind(i),:);
    pt_to = estimated_sources(i,:);
...
true_positives = estimated_sources(tf_positives_association_ind~=0,:);
...
while cond_association
    ...
    for i = 1:numel(high_counts)
        ...
        this_true_source = true_sources(high_counts(i),:);
    ...
    undecided_positives = estimated_sources(tf_positives_association_ind==-1,:);
    undecided_dists = fPathDist2(undecided_positives,true_sources,map_env);    
    for i = 1:size(undecided_positives,1)
        
        this_undecide_dists = undecided_dists(i,:);
        [dist_val,dist_ind] = sort(this_undecide_dists);        
        for j = 1:size(true_sources,1)
            current_dist =...
...
%-- final stuff
true_positives = estimated_sources(tf_positives_association_ind~=0,:);
false_positives = estimated_sources(tf_positives_association_ind==0,:);
...
map_positive((map_recon(:)>para_.concentrationThresholdEvaluation)) = 1;
